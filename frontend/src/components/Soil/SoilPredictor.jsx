...
import {predictSoilType,predictSoilQuality,} from "../../services/api";
...
function SoilPredictor({ onPredict }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const [soilTypeResponse,soilQualityResponse,] = await Promise.all([
        predictSoilType(soilTypePayload),
        predictSoilQuality(soilQualityPayload),
      ]);

      console.log("SOIL TYPE RESPONSE:",soilTypeResponse);
      console.log("SOIL QUALITY RESPONSE:",soilQualityResponse);

      const soilTypes =(soilTypeResponse.top_3_soil_types || []).slice(0, 2).map((soil) => ({
            name: soil.soil_type,
            confidence: Number(soil.probability),
          }));

      const soilQualityScore =soilQualityResponse.soil_quality_score;
      console.log("FORMATTED SOIL TYPES:",soilTypes);
      console.log("OVERALL SOIL QUALITY SCORE:",soilQualityScore);
      onPredict({soilTypes: soilTypes,soilQualityScore: soilQualityScore,});
    } catch (err) {
      console.error(
        "Prediction Error:",
        err
    ...
    <section className="predictor-page">
      <div className="predictor-container">
        ...
        <form className="soil-form" onSubmit={handleSubmit}>
          <div className="crop-select-field">
            ...
            <select id="crop" name="crop" value={form.crop} onChange={handleCropChange}>
              <option value="Rice">
                Rice
              </option>
              <option value="Wheat">
          ...
          {soilFields.map((field) => (
            <RangeField key={field.name} {...field} value={form[field.name]} onChange={handleChange}/>
          ))}
      ...
      <style>{`
        ...
        .soil-form {
          ...
          gap: 32px;
          box-shadow:0 30px 80px rgba(71,51,34,.08);
        }
        .crop-select-field {
          grid-column: 1 / -1;
          display: flex;
        ...
        .predict-btn:hover:not(:disabled) {
          ...
          background: #657b57;
          box-shadow: 0 15px 30px rgba(80,100,70,.25);
        }
