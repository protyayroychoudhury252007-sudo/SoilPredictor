...
electrical_conductivity = np.random.uniform(0.1,8,NUM_SAMPLES)
...
nitrogen_score = np.clip(100 - abs(nitrogen - 100) * 0.8,0,100)
phosphorus_score = np.clip(100 - abs(phosphorus - 50) * 1.5,0,100)
potassium_score = np.clip(100 - abs(potassium - 100) * 0.8,0,100)
ph_score = np.clip(100 - abs(ph_value - 6.5) * 25,0,100)
moisture_score = np.clip(100 - abs(soil_moisture - 30) * 2.5,0,100)
organic_carbon_score = np.clip(organic_carbon * 30,0,100)
ec_score = np.clip(100 - electrical_conductivity * 12,0,100)
temperature_score = np.clip(100 - abs(temperature - 25) * 5,0,100)
humidity_score = np.clip(100 - abs(humidity - 60) * 1.2,0,100)
rainfall_score = np.clip(100 - abs(rainfall - 1200) / 20,0100)
...
soil_quality += np.random.normal(0,3,NUM_SAMPLES)
soil_quality = np.clip(soil_quality,0,100)
...
DATASET_PATH = (DATASET_DIR /"soil_quality_dataset.csv")
